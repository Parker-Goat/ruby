...
-- 🔴 BORDA VERMELHA
UIStroke.Color = Color3.fromRGB(120, 80, 255)  -- ACCENT
...
TextLabel.Text = "RUBY // ACCESS"
TextLabel.TextColor3 = Color3.fromRGB(180, 120, 255)
...
t2.value22.Text = "VERIFICAR KEY"
t2.value22.BackgroundColor3 = Color3.fromRGB(95, 55, 180)  -- ACCENT
...
-- A conexao do botao fica registrada antes do AUTO LOGIN para que uma
-- chave salva que demore/falhe nao bloqueie a verificacao manual.
...
if v5 and v5 ~= "" then
    t2.value22.Text = "TENTANDO LOGIN..."
    task.spawn(function()
        local ok, valid, reason = pcall(function()
            return t2.value17(v5)
        end)

        if not ok then
            valid = false
            reason = "AUTO LOGIN FALHOU"
        end
        ...
        if valid then
            ...
            print("RUBY HUB: AUTO LOGIN")
        else
            t2.value15(t2.value11)
            if t2.value22 and t2.value22.Parent then
                t2.value22.Text = "ATIVAR LICENCA"
                if reason and reason ~= "" then
                    TextLabel2.Text = tostring(t2.value18(reason))
...
local colorSequenceKeypoint6 = ColorSequenceKeypoint.new(0, Color3.fromRGB(9, 12, 24))
...
t2.value120.Color = ColorSequence.new({
	...
	ColorSequenceKeypoint.new(1, Color3.fromRGB(24, 17, 42))
})
