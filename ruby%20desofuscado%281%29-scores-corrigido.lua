...
local function RubyScoreStart()
    ...
    local okService = pcall(function()
        BallService = t2.value5.Packages._Index["sleitnick_knit@1.7.0"].knit.Services.BallService
    end)

    if not okService or not BallService then
        return
    end

    local HitGround = BallService.RE and BallService.RE:FindFirstChild("HitGround")
    if not HitGround then
        return
    end

    local function triggerScore(position)
        if not RubyScoreEnabled or not firesignal then
            return
        end

        local localPlayer = t2.value23
        if not localPlayer then
            return
        end

        local characterModel = t2.value6:FindFirstChild(localPlayer.Name)
    ...
    RubyScoreConnections[1] = t2.value6.ChildAdded:Connect(function(child)
        if child.Name ~= "HitIndicator" then
            return
        end

        if child.Color == Color3.fromRGB(0, 255, 0) then
            triggerScore(child.Position)
        end
    end)

    RubyScoreConnections[2] = t2.value6.DescendantAdded:Connect(function(descendant)
        if not descendant:IsA("BasePart") or descendant.Name ~= "HitIndicator" then
            return
        end

        if descendant.Color == Color3.fromRGB(0, 255, 0) then
            triggerScore(descendant.Position)
        end
    end)

    local currentIndicator = t2.value6:FindFirstChild("HitIndicator")
    if currentIndicator and currentIndicator.Color == Color3.fromRGB(0, 255, 0) then
        triggerScore(currentIndicator.Position)
    end
end
...
local function RubyPlayerCardStart()
    ...
    local PlayerCardService
    local okService = pcall(function()
        PlayerCardService = t2.value5.Packages._Index["sleitnick_knit@1.7.0"].knit.Services.PlayerCardService
    end)

    if not okService or not PlayerCardService then
        return
    end

    local OnFlashCard = PlayerCardService.RE and PlayerCardService.RE:FindFirstChild("OnFlashCard")
    if not OnFlashCard then
        return
    end

    local function triggerCard()
        if not RubyPlayerCardEnabled or RubyPlayerCardBusy or not firesignal then
            return
        end
...
-- Conteúdo da aba SCORES já criada junto das outras abas.
...
v67(RubyScoresFrame, "ScoreEffect Changer", "rubyScoreEffectEnabled", function(enabled)
    RubyScoreEnabled = enabled

    if enabled then
        RubyScoreStart()
        if t2.value38 then
            t2.value38("SCORE EFFECT: ON")
        end
    else
        RubyScoreStop()
        if t2.value38 then
            t2.value38("SCORE EFFECT: OFF")
        end
    end
end)
...
v67(RubyScoresFrame, "PlayerCard Changer", "rubyPlayerCardEnabled", function(enabled)
    RubyPlayerCardEnabled = enabled

    if enabled then
        RubyPlayerCardStart()
        if t2.value38 then
            t2.value38("PLAYER CARD: ON")
        end
    else
        RubyPlayerCardStop()
        if t2.value38 then
            t2.value38("PLAYER CARD: OFF")
        end
    end
end)
