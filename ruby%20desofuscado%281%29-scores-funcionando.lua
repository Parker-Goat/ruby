...
if _CoreGui:FindFirstChild("RUBYHUBPremiumUI") then
    return
end
local _OldKeySystem = _CoreGui:FindFirstChild("RUBYHUBKeySystem")
if _OldKeySystem then
    _OldKeySystem:Destroy()
end
...
local RubyAppId = "ruby_iyghk39o"
local RubyVersion = "1.1"
t2.value10 = "https://rubymanage-jx6h3px2.manus.space"
t2.value11 = nil
t2.value12 = "RUBYHWID.txt"
t2.value13 = function(p1)
...
local function v4()
    ...
    if not ClientId or tostring(ClientId) == "" then
        ClientId = "RUBY-" .. tostring(game:GetService("Players").LocalPlayer.UserId)
    end
    ClientId = tostring(ClientId):gsub("%s+", "")
    t2.value14(t2.value12, ClientId)
    return ClientId
end
function t2.value16()
    ...
    if not _request and syn then
        _request = syn.request
    end
    if not _request and fluxus then
        _request = fluxus.request
    end
    return _request
end
local RubySession = nil
local function rubyRequest(method, path, body)
    ...
    local ok, response = pcall(function()
        return executorRequest({
            ...
            Body = body and t2.value8:JSONEncode(body) or nil
        })
    end)
    ...
    if statusCode and statusCode >= 400 then
        local decoded
        pcall(function()
            decoded = t2.value8:JSONDecode(responseBody)
        end)
        return decoded or nil, "HTTP_" .. tostring(statusCode)
    end

    local decodedOk, decoded = pcall(function()
        return t2.value8:JSONDecode(responseBody)
    end)
...
function t2.value17(p5)
    ...
    local ok, accepted, status = pcall(function()
        if not RubySession then
            local initData, initError = rubyRequest("POST", "/api/v1/auth/init", {
                appId = RubyAppId,
                version = RubyVersion
            })

            if not initData or initData.success ~= true or not initData.session then
                return false, tostring((initData and initData.message) or initError or "API_ERROR")
            end

            RubySession = tostring(initData.session)
        end

        local data, requestError = rubyRequest("POST", "/api/v1/auth/validate", {
            appId = RubyAppId,
            key = key,
            version = RubyVersion,
            session = RubySession,
        ...
        if data and data.success == true and data.license and data.license.status == "active" then
            return true, "VALID"
        end

        local message = tostring((data and data.message) or requestError or "INVALID")

        -- Se a sessão estiver inválida, tenta um novo init uma única vez.
        if requestError == "HTTP_400" or message:lower():find("valid session", 1, true) then
            RubySession = nil
            local retryInit, retryError = rubyRequest("POST", "/api/v1/auth/init", {
                appId = RubyAppId,
                version = RubyVersion
            })
            if retryInit and retryInit.success == true and retryInit.session then
                RubySession = tostring(retryInit.session)
                local retryData, retryRequestError = rubyRequest("POST", "/api/v1/auth/validate", {
                    appId = RubyAppId,
                    key = key,
                    version = RubyVersion,
                    session = RubySession,
                ...
                if retryData and retryData.success == true and retryData.license and retryData.license.status == "active" then
                    return true, "VALID"
                end
                return false, tostring((retryData and retryData.message) or retryRequestError or retryError or "INVALID")
            end
        end
...
Frame.Position = UDim2.new(0.5, -190, 0.5, -98)
...
-- 🔴 BORDA VERMELHA
UIStroke.Color = Color3.fromRGB(0, 220, 190)  -- ACCENT
UIStroke.Thickness = 3  -- MAIS ESPESSA PARA DESTACAR
...
TextLabel.Text = "RUBY"
TextLabel.TextColor3 = Color3.fromRGB(0, 220, 190)
...
t2.value22.Text = "valide a key"
t2.value22.BackgroundColor3 = Color3.fromRGB(0, 150, 170)  -- ACCENT
...
t2.value22.MouseButton1Click:Connect(function()
    ...
    if v174 == "" then
        t2.value22.Text = "DIGITE SUA KEY"
        return
    end
...
t1.value2 = t1.value1(155, 17, 30)
value26.ThemeColor = Color3.fromRGB(155, 17, 30)
...
t2.value59 = nil
t2.value60 = {
	"RUBYHUBConfig_V2.json",  -- ALTERADO
	"RUBYHUBConfig.json",      -- ALTERADO
	"RUBYHUBConfig_V2.json"    -- ALTERADO
}
...
local function v18(p10)
    ...
    return Color3.fromRGB(155, 17, 30)
end
...
function t2.value67()
    if not writefile then
        warn("[RUBY HUB SAVE] writefile não existe nesse executor")  -- ALTERADO
    ...
    if not ok or not v250 then
        warn("[RUBY HUB SAVE] Erro ao gerar JSON:", v250)  -- ALTERADO
    ...
    for _, v in ipairs(t2.value60) do
        if v and v ~= "" then
            ...
            if success then
                ...
                v252 = result
                warn("[RUBY HUB SAVE] Falhou em", v, result)  -- ALTERADO
            end
        end
    end
    if not v251 then
        warn("[RUBY HUB SAVE] Nenhum arquivo salvou:", v252)  -- ALTERADO
...
function t2.value68()
    if not isfile or not readfile then
        warn("[RUBY HUB SAVE] isfile/readfile não existe nesse executor")  -- ALTERADO
    ...
    for _, v in ipairs(t2.value60) do
        ...
        if v234 and (v234 ~= "" and isfile(v234)) then
            ...
            if ok and (type(v237) == "string" and v237 ~= "") then
                ...
                warn("[RUBY HUB SAVE] Config corrompida em", v234)  -- ALTERADO
            end
        end
    end
...
function t2.value76()
    ...
    if not ok then
        warn("[RUBY HUB JERSEY] Erro ao aplicar roupa:", result)  -- ALTERADO
    end
end
local function v23()
    t2.value75()
...
function t2.value98(p28, p29, p30)
    ...
    Part.Name = "RUBYHUBJerseyOutfit"  -- ALTERADO
...
function t2.value99(p31, p32, p33, p34)
    ...
    Part.Name = "RUBYHUBJerseyDetail"  -- ALTERADO
    Part.Size = p33
    Part.Color = p34
    Part.Material = Enum.Material.SmoothPlastic
    Part.Transparency = 0
            ...
            if player ~= t2.value23 and player.Character then
                ...
                if Head and HumanoidRootPart then
                    if not t2.value28[player] or not t2.value28[player].Parent then
                        ...
                        Part.Name = "RUBYHUBESPLine"  -- ALTERADO
                        Part.Anchored = true
...
local function v35()
    ...
    if Character then
        ...
        if not (v413 and v413:GetAttribute("RUBYHUB_StartC0")) then  -- ALTERADO
        end
...
function t2.value102()
    ...
    t2.value101 = t2.value3.RenderStepped:Connect(function(dt)
        ...
        u408 = u408 + (v407 - u408) * math.min(1, 7 * dt)
...
t1.value2.Name = "RUBYHUBPremiumUI"  -- ALTERADO
t1.value2 = t2.value104
...
t2.value107.Size = UDim2.new(0, 60, 0, 60)
...
UIStroke3.Color = t2.value26.ThemeColor  -- ACCENT RUBY
...
t1.value1 = ColorSequenceKeypoint.new(0, Color3.fromRGB(0, 220, 190))

local colorSequenceKeypoint = ColorSequenceKeypoint.new(0.15, Color3.fromRGB(155, 17, 30))
local colorSequenceKeypoint2 = ColorSequenceKeypoint.new(0.32, Color3.fromRGB(190, 24, 45))
local colorSequenceKeypoint3 = ColorSequenceKeypoint.new(0.5, Color3.fromRGB(0, 255, 200))
local colorSequenceKeypoint4 = ColorSequenceKeypoint.new(0.68, Color3.fromRGB(40, 120, 255))
local colorSequenceKeypoint5 = ColorSequenceKeypoint.new(0.84, Color3.fromRGB(120, 70, 255))
...
local function v57(p45)
    if not p45 or p45:GetAttribute("RUBYHUBDraggable") then  -- ALTERADO
        return
    end
    p45:SetAttribute("RUBYHUBDraggable", true)  -- ALTERADO
    local u441 = false
    local inputPosition
    local p45Position
...
t1.value2 = t2.value114
t1.value2.Size = t2.value25 and UDim2.new(0, 760, 0, 480) or UDim2.new(0, 860, 0, 540)
t1.value2 = t2.value114
t1.value2.Position = t2.value25 and UDim2.new(0.5, -380, 0.5, -240) or UDim2.new(0.5, -430, 0.5, -270)
t1.value2 = t2.value114
...
t2.value116.Color = Color3.fromRGB(0, 210, 255)
...
t2.value120.Color = ColorSequence.new({
	ColorSequenceKeypoint.new(0, Color3.fromRGB(8, 24, 34)),
	ColorSequenceKeypoint.new(1, Color3.fromRGB(14, 36, 50))
})
...
t2.value127.Size = UDim2.new(1, -28, 1, -138)
t2.value127.Position = UDim2.new(0, 14, 0, 128)
t2.value127.BackgroundTransparency = 1
t2.value127.Parent = t2.value114
t2.value128 = Instance.new("Frame")
t2.value128.Name = "RUBYHUBSkinPreviewPanel"  -- ALTERADO
...
Frame5.Name = "RUBYHUBAvatarFrame"  -- ALTERADO
Frame5.Size = UDim2.new(1, -24, 0, 300)
Frame5.Position = UDim2.new(0, 12, 0, 50)
Frame5.BackgroundColor3 = Color3.fromRGB(12, 18, 26)
...
t2.value132.Name = "RUBYHUBPlayerSkin"  -- ALTERADO
...
local function v67(p46, p47, p48, p49)
    ...
    Frame6.BackgroundColor3 = Color3.fromRGB(10, 16, 27)
    ...
    TextButton.BackgroundColor3 = Color3.fromRGB(31, 43, 56)
    TextButton.Text = ""
    ...
    local function v461(p50)
        ...
        t2.value1:Create(TextButton, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
			BackgroundColor3 = u460 and Color3.fromRGB(255, 0, 0) or Color3.fromRGB(31, 43, 56)  -- VERMELHO
		}):Play()
...
function t1.value2(p51, p52, p53, p54, p55, p56, p57)
    ...
    Frame8.BackgroundColor3 = Color3.fromRGB(10, 16, 27)
    ...
    Frame9.BackgroundColor3 = Color3.fromRGB(31, 43, 56)
...
local function v68(p59)
    ...
    TextButton.Size = UDim2.new(0, 110, 1, 0)
...
local function v69(p60, p61, p62, p63, p64)
    ...
    for v500, v501 in ipairs(p62) do
        ...
        TextButton.MouseButton1Click:Connect(function()
            ...
            -- Mantém a cor Ruby ao trocar de aba.
            if t2.value174 then
                t2.value174()
            end
        end)
...
-- SCORES: mesma estrutura de navegação das demais abas.
local RubyScoresButton, RubyScoresFrame = v68("SCORES")
t2.value180 = RubyScoresButton
t2.value181 = RubyScoresFrame

-- ============================================================
-- RUBY / SCORES
-- Adicionado diretamente em "ruby desofuscado(1).lua".
-- Baseado nas funções de ScoreEffect/PlayerCard de 7m pago.txt.
-- ============================================================

local RubyScoreEffects = {
    "SupernovaScoreEffect",
    "MoonlightBeamScoreEffect",
    "VioletBeamScoreEffect",
    "Season10ProScoreEffect"
}

local RubyScoreEffectSelected = "SupernovaScoreEffect"
local RubyScoreEnabled = false
local RubyScoreConnections = {}
local RubyPlayerCardEnabled = false
local RubyPlayerCardBusy = false

-- Usa exatamente os remotes/argumentos da source 7m pago.txt,
-- adaptados aos objetos já existentes neste Ruby.
local function RubyScoreStart()
    if RubyScoreConnections[1] then
        return
    end

    RubyScoreEnabled = true

    local ok, HitGround = pcall(function()
        return t2.value5.Packages._Index["sleitnick_knit@1.7.0"].knit.Services.BallService.RE.HitGround
    end)

    if not ok or not HitGround then
        RubyScoreEnabled = false
        if t2.value38 then
            t2.value38("ScoreEffect: HitGround não encontrado")
        end
        return
    end

    local playerName = t2.value23.Name

    local function RubyTriggerScore(position)
        if not RubyScoreEnabled or not firesignal then
            return
        end

        local characterModel = t2.value6:FindFirstChild(playerName)
        if not characterModel then
            characterModel = t2.value6:FindFirstChild("uwuwuwiiwuw6")
        end
        ...
        pcall(function()
            firesignal(HitGround.OnClientEvent, table.unpack({
                ...
                false,
                5,
                RubyScoreEffectSelected,
                characterModel,
                Vector3.new(-2.0885916001134E-06, 1, -4.710930170404E-06)
            }))
        end)
    end

    local function RubyCheckIndicator(indicator)
        if not indicator or indicator.Name ~= "HitIndicator" then
            return
        end

        if not indicator:IsA("BasePart") then
            return
        end

        -- Mesma condição da source: somente indicador verde.
        if indicator.Color == Color3.fromRGB(255, 0, 0) then
            return
        end

        if indicator.Color == Color3.fromRGB(0, 255, 0) then
            RubyTriggerScore(indicator.Position)
        end
    end

    RubyScoreConnections[1] = t2.value6.ChildAdded:Connect(RubyCheckIndicator)
    RubyScoreConnections[2] = t2.value6.DescendantAdded:Connect(RubyCheckIndicator)

    local existing = t2.value6:FindFirstChild("HitIndicator")
    if existing then
        RubyCheckIndicator(existing)
    end
end

local function RubyScoreStop()
    RubyScoreEnabled = false

    for i = 1, 2 do
        if RubyScoreConnections[i] then
            RubyScoreConnections[i]:Disconnect()
            RubyScoreConnections[i] = nil
        end
    end
end

local function RubyPlayerCardStart()
    if RubyScoreConnections[3] then
        return
    end

    RubyPlayerCardEnabled = true
    RubyPlayerCardBusy = false

    local ok, OnFlashCard = pcall(function()
        return t2.value5.Packages._Index["sleitnick_knit@1.7.0"].knit.Services.PlayerCardService.RE.OnFlashCard
    end)

    if not ok or not OnFlashCard then
        RubyPlayerCardEnabled = false
        if t2.value38 then
            t2.value38("PlayerCard: OnFlashCard não encontrado")
        end
        return
    end

    local function RubyTriggerCard()
        if RubyPlayerCardBusy or not RubyPlayerCardEnabled or not firesignal then
            return
        end

        RubyPlayerCardBusy = true

        pcall(function()
            firesignal(OnFlashCard.OnClientEvent, {
                PlayerCardItemId = "UltimateChampionPlayerCard",
                Context = {
                    Custom = {},
    ...
    RubyScoreConnections[3] = t2.value23.PlayerGui.DescendantAdded:Connect(function(descendant)
        if descendant.Name ~= "FlashPlayerCardFrame" then
            return
        end

        if not RubyPlayerCardEnabled or RubyPlayerCardBusy then
            return
        end
    ...
    RubyScoreConnections[4] = t2.value23.PlayerGui.DescendantRemoving:Connect(function(descendant)
        if descendant.Name == "FlashPlayerCardFrame" then
            RubyPlayerCardBusy = false
        end
    end)

    local interface = t2.value23.PlayerGui:FindFirstChild("Interface")
    if interface then
        local persistent = interface:FindFirstChild("Persistent")
        if persistent then
            local flash = persistent:FindFirstChild("FlashPlayerCardFrame")
            if flash then
                pcall(function()
                    flash:Destroy()
                end)
...
local function RubyPlayerCardStop()
    RubyPlayerCardEnabled = false
    RubyPlayerCardBusy = false
...
-- Conteúdo da aba SCORES já registrada no sistema de navegação.
RubyScoresButton = t2.value180
RubyScoresFrame = t2.value181

v67(RubyScoresFrame, "ScoreEffect Changer", "rubyScoreEffectEnabled", function(enabled)
    if enabled then
        RubyScoreStart()
        if t2.value38 then
            t2.value38("ScoreEffect Changer: ENABLED")
        end
    else
        RubyScoreStop()
        if t2.value38 then
            t2.value38("ScoreEffect Changer: DISABLED")
        end
    end
end)

v69(RubyScoresFrame, "Effect Select", RubyScoreEffects, "rubyScoreEffect", function(value)
    RubyScoreEffectSelected = value

    if RubyScoreEnabled then
        RubyScoreStop()
        task.wait()
        RubyScoreStart()
    end
...
v67(RubyScoresFrame, "PlayerCard Changer", "rubyPlayerCardEnabled", function(enabled)
    if enabled then
        RubyPlayerCardStart()
        if t2.value38 then
            t2.value38("PlayerCard Changer: ENABLED")
        end
    else
        RubyPlayerCardStop()
        if t2.value38 then
            t2.value38("PlayerCard Changer: DISABLED")
        end
    end
end)
...
t1.value1.BackgroundColor3 = Color3.fromRGB(10, 16, 27)
...
t2.value152.Name = "RUBYHUBThemeButton"  -- ALTERADO
...
t2.value154.Name = "RUBYHUBThemeButton"  -- ALTERADO
...
t2.value156.Name = "RUBYHUBThemeButton"  -- ALTERADO
...
local function v92()
    t2.value158.BackgroundColor3 = t2.value26.AutoLongDir == "ESQUERDA" and Color3.fromRGB(255, 0, 0) or Color3.fromRGB(31, 43, 56)  -- VERMELHO
    t2.value159.BackgroundColor3 = t2.value26.AutoLongDir == "DIREITA" and Color3.fromRGB(255, 0, 0) or Color3.fromRGB(31, 43, 56)  -- VERMELHO
end
v92()
...
t2.value160.Name = "RUBYHUBThemeButton"  -- ALTERADO
...
function t2.value56()
    if t2.value39 then
        ...
        t2.value39.BackgroundColor3 = t2.value26.HitboxEnabled and Color3.fromRGB(255, 0, 0) or Color3.fromRGB(10, 16, 27)  -- VERMELHO
...
t2.value39.Name = "RUBYHUBHitboxTouchButton"  -- ALTERADO
t2.value39.Size = UDim2.new(0, 82, 0, 34)
t2.value39.Position = UDim2.new(1, -96, 0.62, 0)
t2.value39.BackgroundColor3 = Color3.fromRGB(10, 16, 27)
...
function t2.value58()
    if t2.value41 then
        ...
        value41.BackgroundColor3 = value45 or Color3.fromRGB(10, 16, 27)
...
t2.value41.Name = "RUBYHUBFreezeTouchButton"  -- ALTERADO
t2.value41.Size = UDim2.new(0, 96, 0, 34)
t2.value41.Position = UDim2.new(1, -110, 0.7, 0)
t2.value41.BackgroundColor3 = Color3.fromRGB(10, 16, 27)
...
function t2.value59()
    if t2.value43 then
        ...
        t2.value43.BackgroundColor3 = t2.value26.LeedFeat and Color3.fromRGB(255, 0, 0) or Color3.fromRGB(10, 16, 27)  -- VERMELHO
...
t1.value1 = "Name"
value43[t1.value1] = "RUBYHUBLeedFeatTouchButton"  -- ALTERADO
...
t1.value1 = "BackgroundColor3"
value43_4[t1.value1] = Color3.fromRGB(10, 16, 27)
...
function t2.value49()
    if t2.value47 then
        ...
        t2.value47.BackgroundColor3 = t2.value26.AutoLong and Color3.fromRGB(255, 0, 0) or Color3.fromRGB(10, 16, 27)  -- VERMELHO
...
t2.value47.Name = "RUBYHUBAutoLongTouchButton"  -- ALTERADO
t2.value47.Size = UDim2.new(0, 96, 0, 34)
t2.value47.Position = UDim2.new(1, -110, 0.86, 0)
t2.value47.BackgroundColor3 = Color3.fromRGB(10, 16, 27)
...
t2.value55.Name = "RUBYHUBLockButton"  -- ALTERADO
...
local function v109(p112)
    ...
    if p112 == "PC" and not t2.value25 then
        -- NOVO LAYOUT PC: sidebar larga + painel central + painel de skin separado
                t2.value114.Size = UDim2.new(0, 860, 0, 540)
                t2.value114.Position = t2.value113 or UDim2.new(0.5, -430, 0.5, -270)
                t2.value114.BackgroundTransparency = 0
                t2.value115.CornerRadius = UDim.new(0, 12)
                t2.value116.Thickness = 2
                t2.value114.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
                t2.value118.Size = UDim2.new(1, 0, 0, 50)
                t2.value118.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
                t2.value119.CornerRadius = UDim.new(0, 12)
        ...
        if t2.value120 then
            t2.value120.Color = ColorSequence.new({
                ...
                t2.value124.Size = UDim2.new(0.75, 0, 1, 0)
        ...
        for _, child in ipairs(t2.value125:GetChildren()) do
            if child:IsA("TextButton") then
                child.Size = UDim2.new(0, 110, 1, 0)
        ...
        -- Barra decorativa abaixo do cabeçalho para separar a navegação do conteúdo.
        local divider = t2.value114:FindFirstChild("RUBYVerticalDivider")
        if not divider then
            divider = Instance.new("Frame")
            divider.Name = "RUBYVerticalDivider"
            divider.BorderSizePixel = 0
            divider.BackgroundColor3 = Color3.fromRGB(55, 12, 20)
            divider.Size = UDim2.new(0, 1, 1, -128)
            divider.Position = UDim2.new(0, 214, 0, 112)
            divider.Parent = t2.value114
        end

        local accentBar = t2.value118:FindFirstChild("RUBYAccentBar")
        if not accentBar then
            accentBar = Instance.new("Frame")
            accentBar.Name = "RUBYAccentBar"
            accentBar.BorderSizePixel = 0
            accentBar.BackgroundColor3 = t2.value26.ThemeColor
            accentBar.Size = UDim2.new(0, 110, 0, 4)
            accentBar.Position = UDim2.new(1, -144, 0, 0)
            accentBar.Parent = t2.value118
        end
    else
...
Frame13.BackgroundColor3 = Color3.fromRGB(10, 16, 27)
...
local color3_8 = Color3.fromRGB(255, 215, 0)
local t23 = {
	name = "Amarelo",
	color = color3_8
}
...
function t2.value174()
    local theme = t2.value26.ThemeColor
    if not theme then
        return
    end
    ...
    -- Aplica a cor escolhida aos elementos de destaque da interface.
    -- O fundo geral continua escuro para manter a legibilidade.
    if t2.value116 then
        t2.value116.Color = theme
    end
    ...
    if t2.value118 then
        t2.value118.BackgroundColor3 = Color3.fromRGB(16, 24, 37)
    end

    if t2.value124 then
        t2.value124.TextColor3 = theme
    end

    local activeObjects = {
        t2.value133, t2.value135, t2.value137, t2.value139,
        t2.value141, t2.value143, t2.value145, t2.value148, t2.value180
    }

    for _, obj in ipairs(activeObjects) do
        if obj and obj:IsA("GuiObject") and obj.Visible then
            obj.BackgroundColor3 = theme
        end
    end

    if t2.value127 then
        for _, child in ipairs(t2.value127:GetChildren()) do
            for _, descendant in ipairs(child:GetDescendants()) do
                if descendant:IsA("Frame") and descendant.Name == "SliderFill" then
                    descendant.BackgroundColor3 = theme
                elseif descendant:IsA("TextButton") and descendant.Name == "ToggleButton" then
                    if descendant.BackgroundColor3 ~= Color3.fromRGB(31, 43, 56) then
                        descendant.BackgroundColor3 = theme
                    end
                elseif descendant:IsA("UIStroke") then
                    descendant.Color = theme
                end
            end
        end
    end
    ...
    if t2.value104 then
        for _, obj in ipairs(t2.value104:GetDescendants()) do
            if obj:IsA("UIStroke") then
                obj.Color = theme
            elseif obj:IsA("ScrollingFrame") then
                obj.ScrollBarImageColor3 = theme
            elseif obj:IsA("TextButton") then
                -- Mantém os botões de presets com suas próprias cores.
                if not (Frame14 and obj:IsDescendantOf(Frame14)) then
                    obj.BackgroundColor3 = theme
                    obj.TextColor3 = (theme == Color3.fromRGB(0, 0, 0)) and darkText or white
                end
            ...
            if obj.Name == "SliderFill"
                or obj.Name == "ToggleButton"
                or obj.Name == "RUBYHUBThemeButton" then
                obj.BackgroundColor3 = theme
            end
        end
    end
    ...
    if t2.value35 then
        t2.value35.BackgroundColor3 = theme
    end

    if t2.value36 then
        t2.value36.BackgroundColor3 = Color3.fromRGB(14, 20, 28)
    end

    if t2.value37 then
        t2.value37.Color = theme
    end

    if t2.value129 then
        t2.value129.Color = theme
    end

    if t2.value131 then
        t2.value131.Color = theme
    end

    if t2.value130 then
        t2.value130.TextColor3 = theme
    end
...
Frame15.BackgroundColor3 = Color3.fromRGB(10, 16, 27)
...
Frame16.Name = "RUBYHUBDiscordLinkFrame"  -- ALTERADO
Frame16.Size = UDim2.new(1, 0, 0, 42)
Frame16.BackgroundColor3 = Color3.fromRGB(10, 16, 27)
Frame16.BorderSizePixel = 0
Frame16.Parent = t2.value149
Instance.new("UICorner", Frame16).CornerRadius = UDim.new(0, 12)
t2.value36 = Instance.new("TextButton")
t2.value36.Name = "RUBYHUBDiscordLinkButton"  -- ALTERADO
t2.value36.Size = UDim2.new(1, -20, 0, 28)
t2.value36.Position = UDim2.new(0, 10, 0.5, -14)
t2.value36.BackgroundColor3 = Color3.fromRGB(14, 20, 28)
t2.value36.Text = "🔗 https://discord.gg/FQppuSdwdj"
...
t1.value2 = "Name"
value37[t1.value2] = "RUBYHUBDiscordStroke"  -- ALTERADO
...
local function v144(p115, p116)
    ...
    for v626, v627 in pairs(t2.value125:GetChildren()) do
        ...
        if v627:IsA("TextButton") then
            v627.BackgroundColor3 = Color3.fromRGB(20, 31, 43)
        end
    end
...
t2.value180.MouseButton1Click:Connect(function()
    v144(t2.value180, t2.value181)
end)
...
function t2.value38(p117)
    ...
    Frame17.BackgroundColor3 = Color3.fromRGB(10, 16, 27)
...
(function()
    ...
    if v259 then
        print("[RUBY HUB SAVE] Config carregada:", v258)  -- ALTERADO

        return v259
    end

    print("[RUBY HUB SAVE] Nenhuma config encontrada, usando padrão")  -- ALTERADO
