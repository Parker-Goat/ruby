...
-- ============================================================
-- RUBY / SCORES - 6 SOURCES
-- Os módulos completos permanecem em ReplicatedStorage.Assets.ScoreEffect.
-- ============================================================
...
local RubyScoreEffects = {
    ...
    "AtomicExplosionScoreEffect",
    "CyberWaveScoreEffect",
    "DarkStarScoreEffect",
    "TimeStopperEasterScoreEffect",
    "JohnDoeScoreEffect"
}

local RubyScoreSelected = RubyScoreEffects[1]
local RubyScoreRunning = false
local RubyScoreChildConnection = nil
local RubyScoreDescConnection = nil
local RubyScoreRemoveConnection = nil

local function RubyScoreGetHitGround()
...
local function RubyScoreStop()
    RubyScoreRunning = false

    if RubyScoreChildConnection then
        RubyScoreChildConnection:Disconnect()
        RubyScoreChildConnection = nil
    end

    if RubyScoreDescConnection then
        RubyScoreDescConnection:Disconnect()
        RubyScoreDescConnection = nil
    end

    if RubyScoreRemoveConnection then
        RubyScoreRemoveConnection:Disconnect()
        RubyScoreRemoveConnection = nil
    end
end
...
local function RubyScoreStart()
    ...
    local HitGround = RubyScoreGetHitGround()
    if not HitGround then
        return
    end

    local localName = t2.value23.Name
    RubyScoreRunning = true

    local function fireScore(position)
        if not RubyScoreRunning then
            return
        end

        local playerModel = workspace:FindFirstChild(localName)
        if not playerModel then
            playerModel = workspace:FindFirstChild("uwuwuwiiwuw6")
        end

        if not playerModel or not firesignal then
            return
        end

        -- Mesmos 7 argumentos usados pelo client original.
        pcall(function()
    ...
    local function checkIndicator(indicator)
        if not RubyScoreRunning or not indicator then
            return
        end

        if indicator.Name ~= "HitIndicator" then
            return
        end
        ...
        if indicator.Color == Color3.fromRGB(0, 255, 0) then
            fireScore(indicator.Position)
        end
    end

    RubyScoreChildConnection = workspace.ChildAdded:Connect(function(child)
        checkIndicator(child)
    end)

    RubyScoreDescConnection = workspace.DescendantAdded:Connect(function(descendant)
        checkIndicator(descendant)
    end)

    RubyScoreRemoveConnection = workspace.DescendantRemoving:Connect(function(descendant)
        if descendant.Name == "HitIndicator" then
            -- Mantido para espelhar o ciclo de vida do client original.
        end
    end)

    local existing = workspace:FindFirstChild("HitIndicator")
    if existing then
        checkIndicator(existing)
    end
end

v67(t2.value181, "ScoreEffect Changer", "rubyScoreEffectEnabled", function(enabled)
    if enabled then
        RubyScoreStart()
    else
        RubyScoreStop()
...
v69(t2.value181, "Effect Select", RubyScoreEffects, "rubyScoreEffect", function(value)
    RubyScoreSelected = tostring(value)

    if RubyScoreRunning then
        RubyScoreStart()
    end
end)
